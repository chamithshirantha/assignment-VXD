...
class SubmissionController extends Controller
{
    ...
    public function store(Request $request, $formId)
    {
        ...
        foreach ($form->fields as $field) {
            ...
            // For radio/checkbox/select validate value(s) are present in options (basic)
            if (in_array($field->type, ['radio', 'checkbox', 'select']) && $answers->has($field->id)) {
                $val = $answers->get($field->id)['value'];
                $optionValues = $field->options->pluck('label')->toArray();
                if (in_array($field->type, ['radio', 'select']) && !in_array($val, $optionValues)) {
